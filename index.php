<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="./css/style.css">
    <title>CARNEWS</title>
</head>
<body>
    <div class="wrapper">
        <header class="header">
            <a href="./index.php" class="logo">
                <img src="./img/logo.png" alt="logo">
            </a>
            <nav class="navbar">

                <a href="./html/gibdd.php">ГИБДД</a> 
                <a href="./html/test.php">Тест-драйвы</a> 
                <a href="./html/dtp.php">ДТП</a> 
                <a href="./html/pdd.php">ПДД</a> 
                <a href="./html/shtraf.php">Штрафы</a>   

            </nav>
            
            <div class="poisk">
            <form action="" class="search-bar">
                <input type="search" name="search" pattern=".*\S.*" required>
                <button class="search-btn" type="submit">
                    <span>Search</span>
                </button>
            </form>
            </div>
            <div class="icon">
                <div class="fas fa-bars" id="menu-btn">
                    <img src="./img/burger menu.png" alt="">
                </div>
            </div>
        </header><br>
        <main class="main">
            <div class="bigimg">
                <!-- <p class="cursor typewriter-animation"></p> -->
                
                <a href="">
                    <img src="./img/мерс.jpg" alt="Mercedes-Benz amg gt 63 s" class="merc">
                </a>
            </div>
            <h1 class="h1">&#9135;&#9135;&#9135;&#9135;&#9135;&#9135;&#9135;&#9135;&#9135;&#9135;&#9135;&#9135; НОВОСТИ &#9135;&#9135;&#9135;&#9135;&#9135;&#9135;&#9135;&#9135;&#9135;&#9135;&#9135;&#9135;</h1>
            <div class="sorting">
            </div>
            <div class="news">
                <article class="movie-card">
                    <img src="./img/мерс.jpg" alt="Avatar wallpaper" />
                  
                    <div class="content">
                      <h1>News 1</h1>
                      <div class="infos">
                        <span>&nbsp;&nbsp;Data:&nbsp;&nbsp;&nbsp;&nbsp;3h12</span>
                      </div>

                  
                      <div class="icons">
                        <svg width="25" height="25" viewBox="0 0 25 25" fill="none" xmlns="http://www.w3.org/2000/svg">
                          <path d="M12.7439 22.3037L11.2939 20.9837C6.1439 16.3137 2.7439 13.2237 2.7439 9.45374C2.7439 6.36374 5.1639 3.95374 8.2439 3.95374C9.9839 3.95374 11.6539 4.76374 12.7439 6.03374C13.8339 4.76374 15.5039 3.95374 17.2439 3.95374C20.3239 3.95374 22.7439 6.36374 22.7439 9.45374C22.7439 13.2237 19.3439 16.3137 14.1939 20.9837L12.7439 22.3037Z" fill="white"></path>
                        </svg>
                  


                      </div>
                    </div>
                  </article><br>

                  <article class="movie-card">
                    <img src="./img/мерс.jpg" alt="Avatar wallpaper" />
                  
                    <div class="content">
                      <h1>News 2</h1>
                      <div class="infos">
                        <span>&nbsp;&nbsp;Data:&nbsp;&nbsp;&nbsp;3h12</span>
                      </div>

                  
                      <div class="icons">
                        <svg width="25" height="25" viewBox="0 0 25 25" fill="none" xmlns="http://www.w3.org/2000/svg">
                          <path d="M12.7439 22.3037L11.2939 20.9837C6.1439 16.3137 2.7439 13.2237 2.7439 9.45374C2.7439 6.36374 5.1639 3.95374 8.2439 3.95374C9.9839 3.95374 11.6539 4.76374 12.7439 6.03374C13.8339 4.76374 15.5039 3.95374 17.2439 3.95374C20.3239 3.95374 22.7439 6.36374 22.7439 9.45374C22.7439 13.2237 19.3439 16.3137 14.1939 20.9837L12.7439 22.3037Z" fill="white"></path>
                        </svg>
                  


                      </div>
                    </div>
                  </article><br>

                  <article class="movie-card">
                    <img src="./img/мерс.jpg" alt="Avatar wallpaper" />
                  
                    <div class="content">
                      <h1>News 3</h1>
                      <div class="infos">
                        <span>&nbsp;&nbsp;Data:&nbsp;&nbsp;&nbsp;3h12</span>
                      </div>

                  
                      <div class="icons">
                        <svg width="25" height="25" viewBox="0 0 25 25" fill="none" xmlns="http://www.w3.org/2000/svg">
                          <path d="M12.7439 22.3037L11.2939 20.9837C6.1439 16.3137 2.7439 13.2237 2.7439 9.45374C2.7439 6.36374 5.1639 3.95374 8.2439 3.95374C9.9839 3.95374 11.6539 4.76374 12.7439 6.03374C13.8339 4.76374 15.5039 3.95374 17.2439 3.95374C20.3239 3.95374 22.7439 6.36374 22.7439 9.45374C22.7439 13.2237 19.3439 16.3137 14.1939 20.9837L12.7439 22.3037Z" fill="white"></path>
                        </svg>
                  


                      </div>
                    </div>
                  </article><br>

                  <article class="movie-card">
                    <img src="./img/мерс.jpg" alt="Avatar wallpaper" />
                  
                    <div class="content">
                      <h1>News 4</h1>
                      <div class="infos">
                      <span>&nbsp;&nbsp;Data:&nbsp;&nbsp;&nbsp;3h12</span>
                    </div>
                  
                  
                      <div class="icons">
                        <svg width="25" height="25" viewBox="0 0 25 25" fill="none" xmlns="http://www.w3.org/2000/svg">
                          <path d="M12.7439 22.3037L11.2939 20.9837C6.1439 16.3137 2.7439 13.2237 2.7439 9.45374C2.7439 6.36374 5.1639 3.95374 8.2439 3.95374C9.9839 3.95374 11.6539 4.76374 12.7439 6.03374C13.8339 4.76374 15.5039 3.95374 17.2439 3.95374C20.3239 3.95374 22.7439 6.36374 22.7439 9.45374C22.7439 13.2237 19.3439 16.3137 14.1939 20.9837L12.7439 22.3037Z" fill="white"></path>
                        </svg>
                  


                      </div>
                    </div>
                  </article>
            </div>
        </main><br><br>
<!-- footer -->
        <div class="footer-basic">
        <footer>
            <div class="social">
              <a href="https://vk.com/gurgen_me"><i class="icon ion-social-vkontakte"><img src="./img/vk.png" alt="vk"></i></a>
              <a href="https://github.com/Gurgen-me"><i class="icon ion-social-github"><img src="./img/github.png" alt="git"></i></i></a>
              <a href="https://t.me/carnews93"><i class="icon ion-social-telegram"><img src="./img/tg.png" alt="tg"></i></a>
              <a href="https://www.instagram.com/gurgen.me/"><i class="icon ion-social-instagram"><img src="./img/instagram.png" alt="inst"></i></a>
              <a href="https://www.facebook.com/profile.php?id=100029114281798"><i class="icon ion-social-facebook"><img src="./img/facebook.png" alt="facebook"></i></a>
            </div>

            <p class="copyright">© Melkonyan Gurgen 2023</p>
        </footer>
    </div>
    </div>
    <script src="./js/script.js"></script>
</body>
</html>